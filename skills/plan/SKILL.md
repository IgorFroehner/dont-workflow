...
3. **Clarify technical ambiguities** — BEFORE writing the plan:
   - If there are multiple valid implementation approaches (e.g. polling vs events, sync vs async, new class vs extending existing), ask the user to choose between the approaches. Show a brief comparison (trade-offs, complexity, fit). Mark the recommended approach with "(Recommended)".
   - If non-functional requirements or scope boundaries are unclear, include these as additional questions when they have clear alternatives. Use regular text for open-ended questions.
...
6. **Present a summary** to the user and ask for approval before considering it final. Once approved:
   - Suggest the user compact the context before implementing, to keep things focused. Give them the exact command to run, substituting the actual file paths:
     ...
     ```
   - Then tell them to use the implement step after compacting.
...
## Findings
<This section is updated during implementation with discoveries made during implementation>
```
